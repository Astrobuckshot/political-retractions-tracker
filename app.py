...
# ====================== SECURE API KEYS ======================
# Required: NEWS_API_KEY (newsapi.org)
# Optional: MEDIASTACK_API_KEY (mediastack.com) -- if absent, app just skips that source
...
USAGE_FILE = "mediastack_usage.json"          # tracks calls against the 100/mo free cap
...
# Stricter matching: a retraction-style word must appear close to the
# political keyword (same title/description blob), AND the headline
# must not match a known false-positive pattern (market "correction",
# "no regrets" colloquialisms, sports "retract" injuries, etc).
...
RETRACTION_WORDS = [
    ...
    r"\bfalse(?:ly)? report(?:ed|ing)?\b",
    r"\bwalk(?:s|ed|ing)? back\b",
    r"\bissues? a correction\b",
    r"\bpulled? the (?:story|article)\b",
]

POLITICS_WORDS = [
    "trump", "biden", "harris", "musk", "epstein", "congress", "election",
    "white house", "potus", "senate", "house of representatives",
    "administration", "scandal", "governor", "mayor", "parliament",
    "prime minister", "president", "lawmaker", "politician", "campaign",
    "vote", "ballot", "impeach", "indictment", "lawsuit", "policy",
]

# Phrases that indicate the match is a false positive even if both word
# groups are technically present.
FALSE_POSITIVE_PATTERNS = [
    r"market correction",          # finance, not journalism
    r"stock(?:s)? correct(?:ed|ion)?",
    r"course correction",
    r"no regrets?\b",              # colloquial, not a retraction
    r"i don'?t regret",
    r"corrective (?:exercise|lens|surgery)",
    r"retractable (?:roof|leash|blade)",
    r"spinal correction",
    r"posture correction",
]
...
def is_genuine_retraction(title: str, description: str) -> bool:
    """Stricter check: must hit a retraction word, must hit a politics
    word, and must not match a known false-positive phrase."""
    blob = f"{title or ''} {description or ''}"
    if FALSE_POSITIVE_RE.search(blob):
        return False
    if not RETRACTION_RE.search(title or ""):
        # require the retraction word in the TITLE specifically --
        # cuts down on articles that merely mention an unrelated
        # correction somewhere in the body/description.
        return False
    text_lower = blob.lower()
    if not any(k in text_lower for k in POLITICS_WORDS):
        return False
    return True
...
def categorize_politics(text):
    text = str(text).lower()
    national = {"trump", "biden", "harris", "musk", "epstein", "congress",
                "election", "white house", "potus", "senate", "supreme court"}
    state = {"sacramento", "san francisco", "denver", "chicago", "california",
...
def fetch_newsapi(days_back=14):
    since = (datetime.now() - timedelta(days=days_back)).strftime("%Y-%m-%d")
    query = (
        '(retraction OR correction OR erratum OR "we regret" OR clarifies '
        'OR corrected OR "false report" OR apologizes OR "walks back")'
        ' AND (trump OR biden OR harris OR musk OR epstein OR election OR '
        'scandal OR administration OR congress OR senate OR governor)'
    )
    url = (
        f"https://newsapi.org/v2/everything?q={query}&language=en"
        f"&from={since}&sortBy=publishedAt&apiKey={NEWS_API_KEY}"
    )
    ...
    try:
        ...
        for art in articles:
            ...
            if not title or not is_genuine_retraction(title, desc):
                continue
...
# ==================== Source: Mediastack ====================
@st.cache_data(ttl=3600)
def fetch_mediastack(days_back=14):
    """Mediastack free tier: 100 req/month, 30-min delayed news, HTTP only
    (HTTPS requires a paid plan). Keywords param does an OR match across
    terms, so we run one call with a combined political+retraction keyword
    string and filter strictly client-side to conserve the request quota."""
    if not MEDIASTACK_API_KEY:
        return pd.DataFrame(), 0

    since = (datetime.now() - timedelta(days=days_back)).strftime("%Y-%m-%d")
    keywords = "retraction,correction,erratum,clarifies,apologizes,trump,biden,election,congress"

    url = (
        f"http://api.mediastack.com/v1/news?access_key={MEDIASTACK_API_KEY}"
        f"&keywords={keywords}&languages=en&date={since},{datetime.now().strftime('%Y-%m-%d')}"
        f"&limit=100"
    )
    ...
    try:
        ...
        payload = resp.json()
        if "error" in payload:
            st.warning(f"Mediastack: {payload['error'].get('message', 'unknown error')}")
...
.card-meta { color: #6b7280; font-size: 0.8rem; margin-bottom: 4px; }
.card-title { font-size: 1.0rem; font-weight: 600; margin-bottom: 8px; line-height: 1.3; }
...
.section-label { font-size: 0.75rem; font-weight: 700; color: #374151; text-transform: uppercase; letter-spacing: 0.03em; margin-top: 10px; margin-bottom: 2px; }
.card-source { color: #9ca3af; font-size: 0.72rem; margin-top: 8px; }
h3.col-header { border-bottom: 3px solid; padding-bottom: 8px; margin-bottom: 16px; }
</style>
...
st.sidebar.caption(f"NewsAPI: configured ✅")
if MEDIASTACK_API_KEY:
    pct = min(usage["calls"] / MEDIASTACK_FREE_CAP, 1.0)
    st.sidebar.caption(f"Mediastack: configured ✅")
    st.sidebar.progress(pct, text=f"{usage['calls']} / {MEDIASTACK_FREE_CAP} free calls used this month")
else:
...
st.sidebar.markdown("---")
st.sidebar.caption(
    "**Setup for forks**: add `NEWS_API_KEY` (required) and "
    "`MEDIASTACK_API_KEY` (optional) to your Streamlit secrets. "
...
<div class="card">
  ...
  <div>{row['Correction']}</div>
  <div class="section-label">Original story</div>
  <div>{row['Original_Claim']}</div>
...
st.markdown("---")
st.caption(
    "**Disclaimer**: This app collects self-retractions and corrections from "
    "news outlets via NewsAPI and Mediastack. It is not a fact-checking service. "
    "Filtering reduces false positives but is not guaranteed to be perfectly accurate."
)
